...
QPixmap DiffPulseVoltammetry::GetImage() const {
	return QPixmap(":/GUI/DiffPulseVoltammetry");
}
ExperimentType DiffPulseVoltammetry::GetType() const {
	return ET_DC;
}
QWidget* DiffPulseVoltammetry::CreateUserInput(UserInput &inputs) const {
  ...
  _START_DROP_DOWN_EXT(START_V_VS_OCP_OBJ_NAME, row, 1, 1, 2);
  ...
  ++row;
  _INSERT_RIGHT_ALIGN_COMMENT("with respect to ", row, 0);
  _START_DROP_DOWN_EXT(FINAL_V_VS_OCP_OBJ_NAME, row, 1, 1, 2);
  _ADD_DROP_DOWN_ITEM("open circuit");
  _ADD_DROP_DOWN_ITEM("reference");
  _END_DROP_DOWN();
...
NodesData DiffPulseVoltammetry::GetNodesData(const UserInput &inputs, const CalibrationData &calData, const HardwareVersion &hwVersion) const {
